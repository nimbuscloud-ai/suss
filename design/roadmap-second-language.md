...
The facts-and-rules layer ([`facts-and-rules.md`](../docs/theory/facts-and-rules.md)) is the language-independent half, built and in production. All we ask of a second adapter is that it discover units, emit summaries in the shared IR, and emit the same facts (`entry`, `calls`, `unitEffect`, `throwsDirect`, and the rest). Everything above that layer comes along unchanged: reachability, effect closures, exception flow, cross-boundary checking, the CLI.
