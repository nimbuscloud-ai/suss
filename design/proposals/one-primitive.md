...
## Two bugs with the same cause

We hit two bugs this week with the same cause. The message-bus checker
flagged every recorded-but-unnamed send as an orphan to a queue named
`""`, because it keeps its own channel index instead of using the
shared pairing. And the intent checker contradicts the behavioral
checker on the same run. A pages-api route serving `*` pairs with GET
clients in `suss check`, while `suss check --intent` reports that
nobody serves GET. That happens because the intent side does an
exact-string map lookup on `boundaryKey`, and pairing has since moved
to bucketing first and agreeing second.

Neither bug is in the shared primitive. Both are in a private copy of
it that missed later changes to the original. Each extra implementation
of an agreement rule is one more place for the next drift to happen.
...
with `semanticsAgree`. The private map is deleted. The contradiction
cannot recur, because only one implementation is left.
...
1. **Channel agreement.** The message-bus checker consumes the pairs
   ...
   deleted. The code already promises this consolidation (#174), and
   now two incidents point at it. It goes first.
...
3. **Pairability in the fuzzer.** `everyBoundaryCanPair` lists again,
   ...
   so picks up the protocols it skipped without saying so.
...
5. **Descent rule.** Three pieces of code implement "stop at a nested
   ...
   copy in `extractDependencyCalls`, and a third in the sub-unit
   context. Make it one rule, with the accessor and constructor variant
   folded in. The walk unification (#118) also depends on this, since
   the walks cannot merge until descent means one thing.
...
7. **Cross-package reuse the imports already allow.** appsync
   ...
   manifest-aws handles). It also takes GraphQL types apart at lower
...
Deleting the modules is the guard. To bring a copy back, someone has to
write a new file, and review notices a new file. Review tends to miss
an existing file that grows a copy. "Does it reuse what exists" stays
on the review checklist, and we are not proposing a new gate until a
copy actually comes back.
