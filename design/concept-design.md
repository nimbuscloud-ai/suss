...
Scope note: this is a design record because it's theoretical
scaffolding and work we aspire to, not behaviour we ship. The
production-facing version of these ideas is in
[Kinds of contract](../docs/why/kinds-of-contract.md) (contract
plurality) and
[Boundary semantics](../docs/theory/boundary-semantics.md) (the
structural model), and the one-paragraph summary a reader of the
documentation gets is in
[Prior art](../docs/theory/prior-art.md). Links below that leave for
GitHub go to design records under `design/`, which are proposals and
working notes rather than documentation.
...
- **Cross-boundary syncs**: HTTP, GraphQL, function-call, React
  ...
  the [status design record](./status.md), Phase 9 and the React
  phases).
...
- [Kinds of contract](../docs/why/kinds-of-contract.md): the
  multi-shape contract taxonomy (shipped product framing of
  Jackson's plurality insight).
- [Boundary semantics](../docs/theory/boundary-semantics.md): the
  three-layer transport / semantics / recognition model (structural
  counterpart to sync semantics).
- [The backlog design record](./backlog.md): forward-looking items
  grounded in this framework (intent specs, Arazzo workflows,
  audience annotation, sync-chain identification, failure-mode
  detection, non-call sync packs).
