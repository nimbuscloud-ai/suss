...
The consumer is deliberately written to exercise every shape that came up in testing on production code: `axios.create()` instances with a `baseURL`, destructured responses, template-literal paths, `try`/`catch` with `err.response.status`, a thin path-passthrough wrapper, and reads of fields the spec declares optional.
...
Both boundaries have the `/api/v3` prefix. The spec's `servers[0].url` is `https://petstore3.swagger.io/api/v3`, so the provider's routes are read under `/api/v3`, and both `axios.create` calls in this project set the same `baseURL`, so the consumer's paths are too. A run where one side has the prefix and the other does not pairs nothing and exits non-zero with `nothingPaired`.

### Warnings: provider produces a status the consumer doesn't handle

These are bugs in the consumer, reported as `unhandledProviderCase` at warning severity: whether an uncovered status is a defect depends on intent the code does not state, which is why the run gates on them only with `--fail-on warning`. Petstore declares 200, 400, and 404 for `GET /api/v3/pet/{petId}` (and 200, 400 for `findByStatus`); the consumer ignores 400 in every shape and ignores 404 in two.
...
Petstore's `Pet` schema lists only `name` and `photoUrls` as required. `id` and `status` are optional, so the spec permits a response that omits them and the consumer is implicitly assuming they'll be present.
...
The check is severity `info`, so it doesn't fail CI by default, but it tells you where the consumer is implicitly depending on something the provider doesn't guarantee.

### Unmatched

Run `make check` with `--all` and the output lists the 17 Petstore operations the consumer doesn't touch (`PUT /api/v3/pet`, `POST /api/v3/pet`, `DELETE /api/v3/pet/{petId}`, all the `/api/v3/store/*` and `/api/v3/user/*` endpoints). It also lists `getJson` under "nothing in this run paired with this boundary", as `GET ?`: the wrapper forwards a parameter, so it states no route of its own, and its callers are what get checked.

## What this example deliberately demonstrates

- **`axios.create()` instances**: `api.get(...)` is matched the same as `axios.get(...)`.
- **Base URLs on both sides**: the instance's `baseURL` and the spec's `servers[0].url` both become the `/api/v3` prefix, so the two sides spell one route the same way.
