...
| `--diff BEFORE AFTER` | Compare two summary files. The report leads with the boundaries that moved: the responses that changed, and what a request now reaches or stopped reaching through the calls it makes. Under that, the files with units that moved, listed by name. A transition that only moved in the file is not a change, so an edit above a handler leaves the diff quiet. Takes `--json`, `--changed-files`, `--budget` and `--chain`. |
