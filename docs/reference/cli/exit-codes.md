...
Every code is `0` or `1`. There is no third code to branch on.

| Command | Exits `0` | Exits `1` |
|---|---|---|
| `suss --help`, `suss --version`, `suss` with no command | Always. Any command given `--help` prints the usage and exits `0` too. | Never, though an unknown command name prints the usage and exits `1`. |
