...
- `suss contract`: produce summaries from declared contracts (OpenAPI, CloudFormation, AppSync, GraphQL SDL, committed `.graphql` operation documents, Prisma schema, Storybook CSF3).
...
It aligns with **Daniel Jackson's concept design** at the coarse level: a suss code unit plays the role of a *concept*, and a `BoundaryBinding` plays the role of a *sync*, the wiring between concepts. The difference is direction. Jackson designs concepts top-down with declared purposes; suss derives them bottom-up from existing code. [`internal/concept-design.md`](internal/concept-design.md) works through the mapping and cites the sources.
