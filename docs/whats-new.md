...
**A filter that only hands the request on is spliced in once.** A `before_action` or a middleware whose body ends in a branch neither arm responds from, `set_header if crawler?` say, came out with three pass-through paths: one for each arm and one that hands on outright, all with the same effects. Composition spliced every one of them around every outcome of the action, so a Rails app with ten such filters in front of each action multiplied every action's transitions by hundreds and wrote a summary file measured in gigabytes. A pass-through another pass-through already covers, same effects and no condition the wider one lacks, is left out of the composition now. The Ruby adapter also builds the filter chain the way Rails does: a `before_action` declared again in a subclass moves that filter to the end of the chain with its new options rather than running it twice, and a `skip_before_action` followed by a new declaration puts the filter back.
