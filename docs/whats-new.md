...
**A GitHub Action posts the behavior diff on a pull request.** `nimbuscloud-ai/suss/.github/actions/inspect-diff@main` reads the head and the base of the pull request, runs `inspect --diff` on the two, and posts the result as one comment that it edits on every push. The comment says which units changed behavior and how, whichever lines the diff touched, and the two summary files are kept as an artifact of the run. The [action README](https://github.com/nimbuscloud-ai/suss/tree/main/.github/actions/inspect-diff) has the workflow to copy. The action keeps suss's per-file cache between runs, so the head read covers the files the pull request touched, and when the workflow also runs on a push to `main` it reads each commit ahead of time, so a pull request compares against those summaries instead of reading its base again.

**`inspect --diff` groups what moved by file.** A run that reported a thousand units printed them in the order the extract happened to write, one block each, so a comment on a pull request showed three of them and stopped. The units now sit under the file they are in, a file the change did not touch comes first, and a unit in a file the change did touch prints as one line saying how much moved. Two new flags say what a report may take: `--changed-files` reads the paths a change touched, one per line, and `--budget` caps the report at a number of characters and counts what it left out. The GitHub Action passes both, so the comment spends its room on the units nobody would go looking for.
