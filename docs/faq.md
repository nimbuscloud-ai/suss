...
Observability records what happened at runtime, once. The union of traces is always a subset of reachable behavior, and you find out about drift after the incident. suss derives the structure of behavior statically, so cases that *can* fire in production but haven't yet still appear in the output.
...
Write a `PatternPack`, declarative configuration that tells suss how the framework registers handlers, where status codes attach to responses, what counts as an effect. Most packs are 100-300 lines of data, no fork of the analyzer. See [Write a pack](/guides/writing-a-pack).
