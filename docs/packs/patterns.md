...
Reference for the `PatternPack` interface and every pattern variant a pack can use. For the conceptual overview of what packs are and how they're categorized, see [What a pack is](/packs/what-a-pack-is). For step-by-step instructions on writing a new pack, see [Write a pack](/packs/write-a-pack).
...
`BoundaryBinding` has transport and semantics as separate fields, and [Boundary semantics](/theory/boundary-semantics) explains the split. Pack declarations written before the split are still valid, because the new field was added without changing existing ones.
...
sharing these. See [Read Python or Ruby](/guides/python-and-ruby).
...
Used by the dogfood script, see [the dogfooding record](https://github.com/nimbuscloud-ai/suss/blob/main/design/docs-internal/dogfooding.md), to write per-package contracts to `.suss/suss-summaries.json`. These pair against consumer-side summaries produced by `packageImport`.
