...
Step-by-step instructions for creating a new framework, client, or runtime pack. For the conceptual overview of what packs are, see [What a pack is](/packs/what-a-pack-is). For the full pattern catalogue and interface contracts, see [Pack patterns](/packs/patterns).
...
**Only hardcode what your library defines.** Every identifier a pack hardcodes has to be one that the library the pack is about declares. A wrapper somebody else wrote goes in the pack's options instead, which the CLI fills from the project's [dependency stubs](/guides/teach-a-dependency). An option describing the project's own layout is filled from `-f <pack>=config.json` the same way:
...
Once the pack runs against a project, `suss extract --explain` prints a health block that says which stage it stopped at. [Fix a run that found nothing](/guides/fix-an-empty-run) covers what each code means.
...
- `docs/packs/catalog.md`, so somebody choosing packs for their stack finds yours.
