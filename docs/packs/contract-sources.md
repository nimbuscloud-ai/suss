...
For what "contract" means and how the shapes relate to derived and observed truth, see [Kinds of contract](/why/kinds-of-contract).
...
> **Related reading:**
> - [The problem](/why/the-problem), why behavioral summaries exist
> - [Architecture](/theory/architecture), how packages fit together
> - [Summary format](/reference/summary-format), the IR / wire format
> - [IR types](/reference/ir), type-by-type walkthrough
> - [Kinds of contract](/why/kinds-of-contract), the five shapes of declared contracts suss absorbs (schema, examples, tests, snapshots, design); every shipping contract source today reads the **schema** shape

## What a contract source is, and isn't

A contract source describes the **boundary contract** of a system: the set of observable behaviors a caller can rely on. It does not describe the implementation. Source code, when available, is one way to derive a boundary contract; a contract source derives the same contract from another source of truth.
...
Contract sources draw from one of four categories:
...
Each category has different update cadence and different fidelity characteristics. The IR doesn't distinguish them; `confidence.source: "derived"` covers all four.
...
Stable namespacing means inspect/diff can group related transitions across contract sources without each one inventing its own labels.
...
Contract sources and extracted summaries pair through the same path-normalized matching (`:id` ↔ `{id}`, etc.). A TypeScript axios consumer of `GET /users/{id}` pairs with:

- An extracted handler with the same method + path
- A contract-source-emitted summary from any source format with the same method + path

The checker doesn't differentiate by source. `confidence.source` is preserved on each side so downstream consumers can filter findings by source if needed (e.g., "show me only mismatches against extracted providers, not contract sources").
