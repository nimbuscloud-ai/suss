...
> **Status:** in progress. `@suss/checker` implements all three pairwise checks against status codes, and `suss check <provider.json> <consumer.json>` renders findings as JSON or human-readable output with a non-zero exit on error-severity findings. Provider-side structured body extraction has landed (object/array/primitive literals decompose into `TypeShape` records with spread source preserved), but the checker itself does not yet compare body shapes across a boundary. See [`status.md`](status.md). Broader analysis layers (cross-service graphs, historical tracking, continuous checking) are deliberately **out of scope** for this repository; see [§ Beyond pairwise](#beyond-pairwise).
