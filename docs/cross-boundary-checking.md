...
> **Status:** in progress. `@suss/checker` implements all three pairwise checks against status codes, and `suss check <provider.json> <consumer.json>` renders findings as JSON or human-readable output with a non-zero exit on error-severity findings. Body-shape matching compares provider response bodies against the declared contract shape (ts-rest `c.type<T>()` lifts into `RawDeclaredContract.responses[].body`) with asymmetric subtyping — literal widening, integer ⊂ number, union variance — emitting `providerContractViolation` on mismatch and `lowConfidence` when the shape is indeterminate. Pairwise body comparison across provider/consumer summaries is still TODO. See [`status.md`](status.md). Broader analysis layers (cross-service graphs, historical tracking, continuous checking) are deliberately **out of scope** for this repository; see [§ Beyond pairwise](#beyond-pairwise).
