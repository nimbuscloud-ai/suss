...
**Furthest.** Intent specifications as a comparable artifact alongside derivation; sync-chain composition into named features; audience-indexed summaries ("what's visible to *which* role?"); observation adapters feeding production traces back into the same substrate. The Jackson-grounded arc is tracked in [the forward-looking backlog](/internal/backlog) and grounded theoretically in [concept design](/internal/concept-design). The behavioral summary is the foundation those layers build on.
