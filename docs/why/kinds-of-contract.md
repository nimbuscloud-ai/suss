...
Related: [Cross-boundary checking](/why/cross-boundary-checking) for the checker mechanics, [Contract sources](/packs/contract-sources) for the readers that turn a declared artifact into summaries, [Boundary semantics](/theory/boundary-semantics) for how boundaries themselves vary.
...
Every API boundary has three behavioral contracts whether or not anyone writes them down. The checker compares them pairwise, and each comparison catches a different class of failure. Which checker function fires for which comparison is in [Cross-boundary checking](/why/cross-boundary-checking).
...
Team-authored intent is a kind of truth of its own, with an artifact stream separate from the contract sources above. [Check against your intent](/guides/check-against-intent) covers the two document kinds and the commands that read them.
