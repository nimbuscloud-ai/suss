...
```
~ serves GET /users/{id}  src/routes/users.ts::getUser
...
The report is organized by boundary. Each block is about one route, queue consumer or Lambda, and says what it responds with and what a request reaches through the calls it makes: a line such as `+ reads postgresql:users  through loadUser` means this route now gets to a table it did not before, whichever function down the chain does the reading. The files with units that moved come last, listed by name.
