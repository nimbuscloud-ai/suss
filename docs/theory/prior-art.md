---
title: Prior art
description: What suss takes from Design by Contract, compiler design, formal verification and Daniel Jackson's concept design, and where it parts company with each.
---
...
It aligns with **Daniel Jackson's concept design** at the coarse level: a suss code unit plays the role of a *concept*, and a `BoundaryBinding` plays the role of a *sync*, the wiring between concepts. The difference is direction. Jackson designs concepts top-down with declared purposes; suss derives them bottom-up from existing code. [Concept design](#concept-design) below works through the mapping and cites the sources.
...
The alignment with Jackson is close enough to be worth working through in full.
...
Scope note: this section is theoretical scaffolding and work we aspire
to, not behaviour we ship. The production-facing version of these ideas
is in [Kinds of contract](/why/kinds-of-contract) (contract plurality)
and [Boundary semantics](/theory/boundary-semantics) (the structural
...
[IR types](/reference/ir) and
[Summary format](/reference/summary-format).
...
- [Kinds of contract](/why/kinds-of-contract): the multi-shape contract taxonomy
  (shipped product framing of Jackson's plurality insight).
- [Boundary semantics](/theory/boundary-semantics): the three-layer
