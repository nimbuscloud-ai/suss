...
<svg class="suss-diagram" viewBox="0 0 660 520" role="img" aria-labelledby="layers-title layers-desc">
  ...
  <text class="note" x="330" y="260" text-anchor="middle">187 rules. 14 of them derive stepsTo(x, y, kind): one hop from a value to a value.</text>
  ...
  <text class="note" x="330" y="498" text-anchor="middle">and comesFrom, objectOf, paramAt, resolves: 59 question rules feeding 42 answer relations</text>
</svg>
...
`RESOLUTION_RULES` in `packages/resolution/src/index.ts` is 187 rules.
...
with its twin, and those are not among the 187. The TypeScript adapter
