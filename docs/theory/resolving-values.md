...
<svg class="suss-diagram" viewBox="0 0 660 520" role="img" aria-labelledby="layers-title layers-desc">
  ...
  <text class="note" x="330" y="260" text-anchor="middle">185 rules. 18 of them derive stepsTo(x, y, kind): one hop from a value to a value.</text>
...
`packages/resolution/src/index.ts` contains 185 rules and no code.
18 of them derive `stepsTo(x, y, kind)`, which says the value `x` leads
to the value `y` in one hop. Three of them, for an argument and the two
kinds of property read, are written as `stepsTo` directly. The other
fifteen are written as `hop`, and each gets a `stepsTo` twin, since a
walk under a receiver context reads `hop`. The TypeScript adapter adds a
sixteenth hop, for `.bind`, with its own twin, and that pair is not
among the 185.
...
A value the class body assigns is different. In Python,
`is_admin: bool = False` on a model is only the value an instance
starts with when its constructor is not given one, and the constructor
that fills it is generated by a library the run cannot read. So a read
off the class itself finds that value, and so does a read off a
construction that passes no arguments. A read off a receiver or a
parameter, whose construction the run cannot see, finds the class's
methods and nothing the class body assigns.
