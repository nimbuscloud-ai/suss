...
what Jackson's framework says, where suss maps onto it, where it
...
mental model some role actually holds) while admitting mental
models are plural. Reshuffled taxonomy:
...
A brief primer on suss's vocabulary, so the mapping reads on its
own. Full definitions in [`ir-reference.md`](/ir-reference)
...
| State | The memory that lives behind a concept's actions: shared closures, hooks, module-scoped variables, external stores. Not itself a primitive in the IR; its presence is *inferred* from subjects that trace across transitions to a common origin. |
...
| Feature | A *named* path through the sync graph: multiple actions and syncs composed into a workflow recognisable to an identified audience. No representation of its own in suss yet; the pairing graph contains the raw edges. |
...
- **A transition is a branch of an action, not an action itself.**
  ...
  into its own concept. The corrected reading: the code unit is the
...
4. **Opacity.** Jackson assumes a fully resolved model. suss
   treats opacity as labelled data (opaque predicates, unresolved
   subjects) and propagates it to the sync level: you can detect a
   sync exists without resolving its state-condition component.
...
6. **Reach.** Call-mediated, state-conditioned, and cross-boundary
   syncs map fully. Event-subscription syncs, temporal-ordering
...
- Fusion → one unit whose transitions split into distinct
...
- **Opaque-gated syncs**: the sync exists, but the state-condition
  ...
  without resolving what gates it. This is a precision ceiling,
...
- State invoked but not owned: it belongs to a different concept;
...
- **Backward**: derived summaries are candidate concept-shapes.
  Compared against a PRD, they answer "is there a well-formed
  concept here, or did we ship fused / smeared / phantom code?"
- **Lateral**: multiple intent specs (product PRD, engineering
  design doc, test plan, support runbook) are concept declarations
  for *different audiences*. Disagreement between them is
  signal, not noise.

Today suss has none of this. The closest adjacent item in the
backlog is [intent specs as a structured data
interface](backlog.md#intent-specs). Under the Jackson mapping,
an intent spec is a concept declaration, evaluable under the
...
1. **Opacity reduction keeps compounding.** Every gain in extraction
   precision (predicate decomposition, subject resolution through
