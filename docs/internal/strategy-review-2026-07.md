...
surface (recommendation: all; packs are the extension story and
they're already shaped for it).
...
- **(b) Intent → code verification for AI-generated code**: the
  intent layer's story. An agent writes code from a spec; suss checks
...
with findings triaged (found-real / false-positive / gap). That
artifact also drives the recognizer/discovery fixes that matter,
in priority order observed from production code rather than speculation.
...
schema that never shipped. Each is small; together they undermine
the exact claim the product makes. Two responses, both cheap: fix
