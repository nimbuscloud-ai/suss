...
| 1.7 `useEffect` bodies as code units | ⬜ | Each `useEffect(fn, deps)` becomes an `effect`-kind summary; inputs are deps, outputs are the effect's behavior + cleanup. Shares component identity. Answers: does the IR need a "cleanup sub-unit" or does one effect summary with two transitions suffice? |
