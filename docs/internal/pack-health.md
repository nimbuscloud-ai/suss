...
against `summaries bound`, because a summary with no binding on it
says nothing about which pack made it, so we cannot count it against
one at all. The cost is a coarser diagnosis: a pack that claimed units
and assembled nothing gets told it bound none of them, which points at
binding when the failure was in assembly. To split them we would need
a per-pack count of the summaries built before binding is attempted,
...
it attaches effects to units other packs discovered, so its own
...
list. Leaving them out looks fine by construction, and that turns a
known gap into an invisible one: if Prisma's recognizers stopped
matching tomorrow, a run's output would be byte-identical.

So we built a pair for them. `PackTally` counts the effects each
pack's recognizers returned, and the unit bodies any pack walked in
the files that pack's gate selected. The reasoning: a recognizer only
runs inside a unit some pack discovered, so the bodies walked in gated
files are what the pack had a chance to match against.

Measuring it killed it. Over the fixtures the pair was live 29 times
and never fired, which made it look like a quiet check. On a private
monorepo it fired on nearly every case where it was live. Opening
those showed that the predecessor is not a fact about the pack at all:
...
effects and looked healthy. Paired instead with a client pack, it
...
There is no cheaper predecessor that fixes this. Only something that
understands the library's API can tell whether a recognizer should
have fired inside a given body, and that something is the recognizer.
Any stand-in at the file level is too loose, because importing a
library into a file whose walked bodies have nothing to do with it is
ordinary code.
...
which is what lets you read those five packs in the funnel instead of
three zeros next to their name. Nothing judges them. **No check here
measures five packs, and those five are Prisma, Drizzle, SQS,
...
**The pack has no gate.** A pack with no gate gets handed every file
in the project, so its candidate count only tells you the project has
files. React Router has no gate, and without this suppression it would
report itself broken on every project that does not use it.
...
say "all fine" for every one of them.
...
what it reads back, and that goes in the summary's metadata. Counting
...
**No declared version.** This one is quiet on any run started through
the CLI, because the CLI hashes the file each pack was loaded from and
folds that hash into the pack's stamp. Eighteen of the nineteen
built-in packs still declare nothing of their own, and only
...
Even while it is quiet, this is the check most likely to get the whole
...
to skim past the lines above it. Each check therefore says who it is
for. A `run` check has found something about the code in front of it,
and it prints whenever it fires. A `pack` check has found something
about how a pack was built, and it waits for `--explain`. The dogfood
run asks for both, because there the person reading is the pack
author.
...
`kind`, which collides within a pack, and a recognizer is a bare
function with no name at all. Giving them an identity means either
asking pack authors for one or threading an index through the adapter.
Neither is worth it, because the check would be wrong most of the time
...
unmatched, and that is normal rather than a fault.

**High confidence on an empty summary.** We built this one and
measured it. It fired three times across everything, and none of the
...
opaque conditions by zero total and treats the result as agreement. A
summary with nothing in it comes out at high confidence. Fixing that
arithmetic is worth doing, and it is not a heuristic.

We have since fixed that arithmetic. The adapter now says what it
...
asking a question the summary answers directly, which is why it is
still not worth running as a heuristic.
...
them. We report their counts and conclude nothing from them.
...
sweeps here produced single-digit drop counts, which is not enough of
a distribution to choose a number that would not be arbitrary. Zero is
the one threshold that needs no evidence, so it is the one we shipped.
...
stage ran. Health therefore only fires on a cold cache or under
