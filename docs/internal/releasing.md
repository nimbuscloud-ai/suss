...
All 46 packages share one version. You raise that number once in the
root `package.json`,
...
lists anything, so a rehearsal that says "would publish 46 packages" is
...
- 46 packages on the registry at the new version.
...
Publishing 46 packages, tagging, and writing the release are three
