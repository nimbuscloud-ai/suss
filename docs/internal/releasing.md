...
All 56 packages share one version. You raise that number once in the
root `package.json`,
...
lists anything, so a rehearsal that says "would publish 56 packages" is
...
- 56 packages on the registry at the new version.
...
Publishing 56 packages, tagging, and writing the release are three
