...
All 57 packages share one version. You raise that number once in the
root `package.json`,
...
lists anything, so a rehearsal that says "would publish 57 packages" is
...
- 57 packages on the registry at the new version.
...
Publishing 57 packages, tagging, and writing the release are three
