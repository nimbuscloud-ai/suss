...
All 45 packages share one version. You raise that number once in the
root `package.json`,
...
lists anything, so a rehearsal that says "would publish 45 packages" is
...
- 45 packages on the registry at the new version.
...
Publishing 45 packages, tagging, and writing the release are three
