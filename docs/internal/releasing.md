...
All 47 packages share one version. You raise that number once in the
root `package.json`,
...
lists anything, so a rehearsal that says "would publish 47 packages" is
...
- 47 packages on the registry at the new version.
...
Publishing 47 packages, tagging, and writing the release are three
