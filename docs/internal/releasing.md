...
All 53 packages share one version. You raise that number once in the
root `package.json`,
...
lists anything, so a rehearsal that says "would publish 53 packages" is
...
- 53 packages on the registry at the new version.
...
Publishing 53 packages, tagging, and writing the release are three
