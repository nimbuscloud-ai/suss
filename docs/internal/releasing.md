...
All 48 packages share one version. You raise that number once in the
root `package.json`,
...
lists anything, so a rehearsal that says "would publish 48 packages" is
...
- 48 packages on the registry at the new version.
...
Publishing 48 packages, tagging, and writing the release are three
