...
All 55 packages share one version. You raise that number once in the
root `package.json`,
...
lists anything, so a rehearsal that says "would publish 55 packages" is
...
- 55 packages on the registry at the new version.
...
Publishing 55 packages, tagging, and writing the release are three
