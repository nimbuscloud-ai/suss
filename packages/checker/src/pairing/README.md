...
- **A key bucket can hold summaries that pair with nothing in it.** A message-bus key carries the subject alone, so `default#order.placed` and `order.placed` land together and a handler that cannot know its bus still meets the template that names one. `bindingsPair` then compares the buses inside the bucket, and two named buses that disagree stay apart. Matching is tracked per summary rather than per key for that reason.
- **`checkAll` does not put message-bus summaries in the unmatched lists.** A channel that paired with nothing is already reported by `checkMessageBus` with a severity and with knowledge of who sends to it. Pairing owns the pair list; that pass owns every judgement about a channel.
