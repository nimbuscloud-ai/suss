...
- `grounding.ts:groundReferences` answers what a unit reaches when the unit was told. A wrapper that takes its bucket as an argument records `{location.bucket}`, and each call in the run already says which summary it reaches and what it passed, so the answer is a join over the summaries rather than another walk over source. A container that is one bare name, `{ORDER_TABLE}`, is a variable the deployment sets rather than an argument anyone passes, so it is grounded against `metadata.runtimeContract.envVarValues` on the runtimes that run the file. `runtime-config/placement.ts:placeRuntimes` works out which those are, and it is the same answer `checkRuntimeConfig` uses to scope a `process.env` read.
