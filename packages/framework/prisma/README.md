...
The recognizer matches a three-segment chain, `<receiver>.<modelDelegate>.<method>(args)`, and verifies the receiver by type. Its symbol declaration, or that of a class it extends, has to live in `@prisma/client`, in `.prisma/client`, or in a directory containing a `schema.prisma`, which covers `const db = new PrismaClient()`, a wrapped `ctx.prisma`, a project's `class PrismaService extends PrismaClient`, and a generator pointed at a directory of the project's own with `output`.

A generator with its own `output` also defeats the import gate, since the code then reaches the client by relative path and nothing imports `@prisma/client`. The pack declares `generatedModuleMarkers: ["schema.prisma"]`, and the adapter treats a relative import into a directory containing that file as an import of the package.
