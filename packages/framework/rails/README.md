...
- **Discovery**: a class whose ancestry reaches `ApplicationController` (or a project's own extra base). Every instance method it defines directly is one of its actions, `index`, `show`, a custom name, whatever the controller writes.
...
- **Method visibility.** Ruby's `private`/`protected` keywords are not read, so a controller's own helper methods are discovered the same way its actions are, with no route and so no boundary. Their calls are still followed, which is the same outcome a reader would get by asking what an unbound action reaches.
