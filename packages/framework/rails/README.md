...
- **Response status**: `render json: item, status: :created` and `head :no_content` both set the status Rails sends, so the pack declares those two calls and where each takes its status, plus Rack's own `SYMBOL_TO_STATUS_CODE` table so that a symbol reads as a number. Rack renamed four of those symbols over its releases, and both spellings of each are declared. An action that sets no status of its own is reported at Rails' default of 200.
...
- **`redirect_to` is not read for a status.** It defaults to 302 where every other action defaults to 200, and the discovery pattern declares one default for the whole controller, so an action that only redirects is still reported at 200.
- **Two statuses down two branches claim neither.** An action that writes `:created` on one path and `:unprocessable_entity` on another gets no status claim and one gap saying which two were found. An action where one branch writes a status and another writes a plain `render` is the same case, with 200 as the second candidate.
- **A helper called by its bare name is followed.** `render json: visible_items` calls `visible_items` on self, so the action gets an invocation effect for it and the helper gets a summary of its own, private or not. The Ruby adapter's README says how a bare name is told apart from a local variable read. One consequence worth expecting: a Rails method the project never defines, `params` or `current_user`, is a call the same way `render` is, and shows up on the summary as an invocation nothing follows.
