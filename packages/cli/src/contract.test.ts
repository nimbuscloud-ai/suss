...
describe("contract CLI command", () => {
  ...
  describe("--from intent", () => {
    ...
    it("loads a single intent file via --from intent", async () => {
      const intentFile = path.join(tmpDir, "users.intent.json");
      fs.writeFileSync(intentFile, JSON.stringify(minimalIntent));
      const writeFn = process.stdout.write;
      process.stdout.write = (() => true) as typeof process.stdout.write;
      let summaries: BehavioralSummary[];
      try {
        summaries = await contract({ from: "intent", spec: intentFile });
      } finally {
        process.stdout.write = writeFn;
      }
      expect(summaries).toHaveLength(1);
    ...
    it("walks a directory of *.intent.json files via --from intent", async () => {
      fs.writeFileSync(
        path.join(tmpDir, "users.intent.json"),
        JSON.stringify(minimalIntent),
      );
      fs.writeFileSync(
      ...
      let summaries: BehavioralSummary[];
      try {
        summaries = await contract({ from: "intent", spec: tmpDir });
      } finally {
        process.stdout.write = writeFn;
      }
    ...
    it("surfaces a useful error for a malformed intent spec", async () => {
      const intentFile = path.join(tmpDir, "broken.intent.json");
      fs.writeFileSync(
        intentFile,
        JSON.stringify({ ...minimalIntent, purpose: "" }),
      );
      await expect(
        contract({ from: "intent", spec: intentFile }),
      ).rejects.toThrow();
    });
  });
