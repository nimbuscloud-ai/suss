...
Every boundary block is a `z.strictObject`, so a misspelt `recieves:` stops the run with `boundary: Unrecognized key: "recieves"` rather than being dropped in silence. Both document shapes are strict for the same reason, so `scenario:` written for `scenarios:` stops the run too. That rejects unknown keys only; the fields that were optional stay optional, so a module-level function-call boundary is still authorable and `source:` can still be left off.
