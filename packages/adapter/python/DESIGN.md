...
An annotation says the name is one of the class, and the adapter records it that way, as `instanceOf(name, annotation)`, for a parameter and for an annotated assignment. A method called on the name then resolves to the one the class declares, and an unannotated helper that its callers pass the name to gets the same class. A field the class body gives a default, such as `is_admin: bool = False`, does not come with it, because a model's constructor fills that field from an argument the run cannot see. The adapter records `callPassesNothing` for a call written with empty parentheses, and only a construction written that way reads those defaults, as `settings = Settings()` does. A class method's first parameter is the class itself, so the adapter binds it to the class, and `cls.TABLE` reads what the class body assigns.
