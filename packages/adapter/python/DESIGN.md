...
A field the class body gives a default, such as `is_admin: bool = False`, is not read through an instance whose construction the run cannot see, because a model's constructor fills that field from an argument. The adapter records `callPassesNothing` for a call written with empty parentheses, and only a construction written that way reads those defaults, as `settings = Settings()` does. A class method's first parameter is the class itself, so the adapter binds it to the class, and `cls.TABLE` reads what the class body assigns.
