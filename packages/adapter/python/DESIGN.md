...
An annotated attribute on a plain class is shared like `TABLE`, since nothing generates a constructor that could replace it. The adapter records it as `holdsDefault` all the same, and says which classes are plain: `plainClass` for a class written with no decorator, no keyword and no base but `object`, and `extendsOnly` for one written the same way over a single base, which the rules find plain when that base is. Any decorator or any other base list leaves the class unrecorded, because `@dataclass` can be imported under another name and a library base can generate a constructor. So a dataclass, a pydantic model, a `NamedTuple` and an attrs class keep their field defaults off the instance without a pack saying anything. A class over a library base that generates nothing, such as `Exception` or `ABC`, stays unread as well.
