...
The value facts record the same methods on the class. An instance method goes under its name, and a class method under the name spelled as a class member, `.x`, so a class that writes `x` both ways keeps the two apart. A call on a constant, or on `self` in a class method, reads the class member, and a call on anything else reads the instance method, which is how the walk above decides too. A call in the class body outside any method reads both, since a block there often runs on an instance. `new` keeps its plain name. So a why question follows `Klass.x` to a `def` inside `class << self` as it does to `def self.x`, and follows `Klass.new.x` to the instance method. Inside a class method the facts give `self` the class's own key. The walk and the facts read a class body through the same helpers in `ast.ts`, `methodsDefinedAt` and `definesClassMethod`, so they agree on which methods it defines and which of those are class methods. The resolution package's DESIGN.md says how the spelling works for every adapter.

A `def` passed to a call defines its method the same as one written on its own line, because Ruby evaluates the argument before the call runs. `private def x`, `protected def x`, `private memoize def x` and `private_class_method def self.x` all define `x`, and `module_function def x` also makes `x` a class method of the module, as `module_function :x` does. Visibility is read from the same calls, so `private def x` keeps `x` out of a controller's actions.
...
The value facts read a bare name the same way, using the same table. `handler = build_index` records a `call`, and `handler` evaluates to what `build_index` returns. It never evaluates to the method itself. Every `a.b` records a `call` too, with the method name as the callee and a `readsProperty` on it, whether or not arguments follow, because Ruby cannot refer to a method by name without running it. That lets the shared rules work out what `Faraday.new` and `Account.first` evaluate to, with no step of the adapter's own for each.
