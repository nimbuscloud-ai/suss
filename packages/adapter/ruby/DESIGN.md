...
The value facts record the same methods on the class, each under its name, so a why question follows `Klass.x` to a `def` inside `class << self` as it does to `def self.x`. Inside a class method the facts give `self` the class's own key. The walk and the facts read a class body through the same helpers in `ast.ts`, `methodsDefinedAt` and `definesClassMethod`, so they agree on which methods it defines and which of those are class methods.
