...
The library calls a field's method with `public_send`, so the method that answers it can be written anywhere in the class's ancestry. `ancestry.ts` walks that chain, each ancestor located by the same constant-to-path convention a `mutation:` or `resolver:` reference already uses. A field is answered by a method of its own name found along that chain; a wired field is answered by the `resolve` method found along the wired class's chain. Which method name a wired class answers with is graphql-ruby's own, so the pack states it as `resolverMethodName`. So is where a project's chain ends: `ancestryRootClassNames` names the library's own root classes, and reaching one means the walk followed everything a project defined.

The order is Ruby's, not an approximation of it. Ruby builds a class's ancestors at include time: it computes each included module's own chain on its own, then inserts that chain as a unit and skips anything already in the ancestors, never relocating a module an earlier include or the superclass already placed. Two concerns sharing a base give `[C, B, A, Base]`, not `[C, B, Base, A]`, and a base the superclass already mixes in stays behind the superclass. So the walk builds the superclass chain first, filters every later step against what is already there, and expands each sibling include on its own rather than descending into it with the set the siblings share. `include A, B` mixes in B before A, while `include A` then `include B` puts B first, so calls read in source order and one call's arguments read backwards.

The differential check for this lives in the tests as behavior, and the linearization itself was checked against `Module#ancestors` in a Ruby process for a diamond, a three-way diamond, a diamond crossing a superclass boundary, `prepend`, multi-argument `include`, a module included twice, and a nested module chain.
...
An ancestor nearer than any definition that nothing could read stops the search, rather than the search carrying on to a method further along. Ruby would have called whatever that unread ancestor holds, so a body found past it is not the answering body, and reporting it would be a confident wrong claim rather than an abstention.
