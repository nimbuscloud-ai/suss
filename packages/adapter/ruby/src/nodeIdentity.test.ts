...
async function twoReadsOfOneNode(): Promise<[RbNode, RbNode]> {
  const tree = await parseRuby("x = 1\n");
  const first = tree.rootNode.namedChildren[0] as RbNode;
  const second = tree.rootNode.namedChildren[0] as RbNode;
  return [first, second];
}
