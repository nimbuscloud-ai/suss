...
function packIsUngated(pack: PatternPack): boolean {
  ...
  // Truly universal recognizers like `@suss/framework-process-env`
  // (process.env is always available) and callback-driven packs that
  // key off something other than imports (a manifest on disk) are the
  // intended consumers of this fallback.
