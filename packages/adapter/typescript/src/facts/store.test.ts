...
describe("resolveCalledFunction", () => {
  ...
  it("runs the target Object.assign hands back", () => {
    expect(
      calledBody({
        "/card.ts": `
          const Root = () => { draw(); };
        ...
        "/use.ts": `
          ...
          export function page() { ReportCard(); }
        `,
      }),
    ).toBe("() => { draw(); }");
  });

  it("does not take a project's own Object for the global one", () => {
    expect(
      calledBody({
        "/use.ts": `
          const Object = { assign: (a: unknown, b: unknown) => b };
          const Root = () => { draw(); };
          const ReportCard = Object.assign(Root, () => { other(); });
          export function page() { ReportCard(); }
        `,
      }),
    ).toBe("() => { other(); }");
