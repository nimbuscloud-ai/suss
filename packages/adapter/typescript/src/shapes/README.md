...
- **Types the project did not declare stop at their name.** A named type whose every declaration sits in the default library or under `node_modules` is reported as `{ type: "ref", name }`, the same shape the depth cap falls back to. A reader who sees `HTMLElement` already knows what came back, and the property set describes the DOM rather than the codebase being read. It is also the difference between a 15MB output file and a 1.3GB one on saleor-dashboard: `seen` only guards a single expansion path, so a dense library graph re-expands the same types through every property until `MAX_DEPTH` runs out. Anonymous types expand wherever they were written, since the compiler names a type literal or a mapped type `__type` and a ref would say less than the fields do. `SourceFile` answers the question, via the program's own default-library and external-library flags rather than a path match.
