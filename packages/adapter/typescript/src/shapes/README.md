...
The extractor calls this layer whenever it needs to know what a value looks like: return-statement bodies (terminals), throw arguments, effect arguments, contract-derived response shapes. It works in three passes: syntactic decomposition first (literals preserve their narrowness), then AST declaration resolution via `resolve/astResolve.ts` (follow imports / variable chains), then type-checker fallback (generics, cross-module types).
...
- **Cross-extractor recursion.** `extractShape → resolveNodeFromAst → extractShape` is a call path that happens. Each entry to `resolveNodeFromAst` resets its own seen-set, so the cycle detection there is per-walk, not per-shape. `shapes.ts` adds a module-local depth counter that drops to the type-checker path past `MAX_EXTRACT_DEPTH`. This caught a crash on adapter-typescript self-extraction.
...
- **Types the project did not declare stop at their name.** A named type whose every declaration lives in the default library or under `node_modules` is reported as `{ type: "ref", name }`, the same form the depth cap falls back to. A reader who sees `HTMLElement` already knows what came back, and the property set describes the DOM rather than the codebase being read. It is also the difference between a 15MB output file and a 1.3GB one on saleor-dashboard: `seen` only guards a single expansion path, so a dense library graph re-expands the same types through every property until `MAX_DEPTH` runs out. Anonymous types expand wherever they were written, since the compiler calls a type literal or a mapped type `__type`, and a ref would say less than the fields do. `SourceFile` settles which case a type is in, using the program's own default-library and external-library flags rather than a path match.
- **Opaque named types.** Date, RegExp, Error, Promise, Map, Set, etc. don't expand, because their internal structure doesn't help you reason about the value. They surface as `{ type: "ref", name: ... }` with the type-checker's text representation (so `Promise<User>` keeps the parameterization in the ref name).
