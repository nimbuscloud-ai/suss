...
This adapter uses the extraction cache shared across every language adapter, from `@suss/extractor`. It supplies the files by reading the tsconfig's include list, and supplies the tsconfig path as the config path that guards the entry alongside the file list. Choosing the cache directory is still each adapter's own job: this one keeps it beside the tsconfig, at `.suss/cache/`, and turns caching off for a caller-supplied `Project` or for `--no-cache` on one run. It also reads mount prefixes back out of each cached file's record to confirm a route's mount still resolves the same way before reusing it. See the extractor package's README for the design.
