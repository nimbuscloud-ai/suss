...
A bare `...UserCard` in a document built by the project's own generated tag (what graphql-codegen's client preset writes, imported as `~/__generated__/gql` or similar) is resolved from the fragment the project defines elsewhere, since that is what the build itself does, and the definition goes into the stored document so the fields selected through it get checked. A document built by a library's tag (`@apollo/client`, `graphql-tag`) is read as written, because that is what ships: a spread it does not define stays in `unresolvedFragments`. A name the project defines more than once with different bodies is used from neither, and comes back in `metadata.graphql.ambiguousFragments` as well. A fragment written in a `.graphql` file is not indexed yet.
