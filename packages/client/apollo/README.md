...
- **Discovery** of hook calls from `@apollo/client` and `@apollo/client/react`: `useQuery`, `useLazyQuery`, `useSuspenseQuery`, `useBackgroundQuery`, `useLoadableQuery`, `useMutation` and `useSubscription`. The query hooks differ in when they run and how they suspend, but they all ask the server for the same thing, so all of them produce the same boundary. The list goes by signature, so a hook missing from it is one the pack cannot see, even though it is still a boundary.
