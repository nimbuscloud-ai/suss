...
- A call is looked up in the rows first. Otherwise, when the lowering
  ...
  parameter gives one name per property, each saying which argument to
  read and which properties to read off it; when that argument does not
  fold to an object with the property, the name is a hole, and when the
  object has no such property, the default applies. Otherwise the
  lowering is asked what
