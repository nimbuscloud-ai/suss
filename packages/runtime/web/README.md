...
`@suss/runtime-web` returns a `FrameworkPack` object describing:
...
This is a "runtime pack", not a "framework pack" — `fetch` is a built-in web API, not a third-party framework. It uses the same `FrameworkPack` interface because the adapter interprets both identically.
