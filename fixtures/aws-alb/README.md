...
Each pattern shape also forwards to both target kinds: the ECS side has a wildcard rule (`/api/orders/*`) and an exact one (`/api/orders/_health`, which doubles as the ALB health check), and the Lambda side has an exact rule (`/api/health`) and a wildcard one (`/api/health/*`). A resolver keyed on pattern shape instead of the target group's `TargetType` picks the wrong backend for two of the four rules.
